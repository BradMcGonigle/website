...
The background features two canvas-based animations that rotate every 8 hours—a dot grid with a pulsing effect and topographic contour lines.

CSS animations would have been simpler for the dot grid. I chose canvas for a few reasons: I wanted both backgrounds to use the same rendering approach for consistency, the topographic animation requires per-pixel control that CSS can't provide, and I hadn't written canvas code in years and wanted to revisit it.

The topographic background uses Perlin noise to generate a height field, then marching squares to extract contour lines at fixed intervals. Marching squares is a standard algorithm for finding isolines in a scalar field—you divide the space into cells, classify each cell based on which corners are above or below the threshold, then draw the appropriate line segment. It's the same technique used in everything from terrain rendering to medical imaging.

Performance was a concern. Both animations target 30fps and use `requestAnimationFrame` with frame skipping if the browser is under load. The dot grid redraws only the dots whose opacity has changed since the last frame. On my test devices, the animations use less than 2% CPU when visible and pause entirely when the tab is backgrounded.

The bundle cost is roughly 4KB gzipped for both animations. Framer Motion would have added ~25KB for arguably simpler code. Whether that trade-off is correct depends on your priorities—I valued the smaller bundle and the learning experience.
...
**Write more content before building.** I spent weeks on infrastructure for a site with seven blog posts. The build system is ready for hundreds of posts that don't exist yet. Starting with a backlog of content would have clarified which features actually mattered.
